...
public class AnimationController : MonoBehaviour
{
	...
	void Start ()
	{
		...
		//ボタンに関するイベントの登録
		scrollViewController.OnButtonDown.AddListener (index => {
			pressedButtonIndex = index;
			isButtonDown = true;
			OnButtonDown (index);
		});

		scrollViewController.OnButtonUp.AddListener (index => {
			isButtonDown = false;
			OnButtonUp (index);
		});
	...
	{
		count = 0;
		NowAnimation = pressedButtonIndex;
